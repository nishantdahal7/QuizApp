...
def completed_session():
    ...
    if (game_score >= qualifying_score):
        ...
        qualify_mssg_label.pack()
        ok_button = Button(top, image=okbtn, cursor="hand2", borderwidth=0, bg="#000000",
                           command=top.destroy)  # , bg="#436fd2"
    show_frame(frame4)
...
def load_highscores(frame):
    ...
    for x in range(0, 3):
        ...
        username_to_load_label = Label(frame5, font=("Courier New", 18, "bold"), text=curr_player, pady=10,
                                       bg="#f8f8f8")
        username_to_load_label.place(x=475, y=disty)

        userscore_to_load_label = Label(frame5, font=("Courier New", 18, "bold"), text=str(curr_score), pady=10,
                                        bg="#f8f8f8")
...
def add_questions():
    ...
    input_question.config(font=("Helvetica", 14), bg="#8319d3", fg="#ffffff", highlightthickness=5,
                          highlightbackground="#74159d", highlightcolor="#74159d")
    # Options
    my_canvas.create_text(153, 180, text="Enter Options :", font=("Helvetica", 18, "bold"), fill="#ffffff", anchor="nw")
    input_options = Entry(top3)
    input_options.config(font=("Helvetica", 14), bg="#8319d3", fg="#ffffff", highlightthickness=5,
                         highlightbackground="#74159d", highlightcolor="#74159d")
    my_canvas.create_window(150, 210, window=input_options, width=750, anchor="nw")
    # Guide
    my_canvas.create_text(153, 255, text="**Enter 4 Options Above", font=("Helvetica", 10, "bold"), fill="#e31313",
                          anchor="nw")
    my_canvas.create_text(153, 273, text="**Enter Comma Seperated values", font=("Helvetica", 10, "bold"),
                          fill="#e31313", anchor="nw")
    # Correct Answers
    my_canvas.create_text(153, 330, text="Correct Option :", font=("Helvetica", 18, "bold"), fill="#ffffff",
                          anchor="nw")
    input_correct_option = Entry(top3)
    input_correct_option.config(font=("Helvetica", 14), bg="#8319d3", fg="#ffffff", highlightthickness=5,
                                highlightbackground="#74159d", highlightcolor="#74159d")
    my_canvas.create_window(153, 360, window=input_correct_option, width=750, anchor="nw")
    my_canvas.create_text(153, 405,
                          text="*ENTER THE CORRECT ANSWER ABOVE(Note: It Must Be Present In The Enter Options Row **CASE SENSITIVE ",
                          font=("Helvetica", 10, "bold"), fill="#e31313", anchor="nw")
    # Submit Button
    submit_btn = Button(my_canvas, image=submit_btn_img, border=0, relief=FLAT, bg="#5f4bd1", cursor="hand2",
                        command=handle_questions_options)
...
def loop_check():
    ...
    success_mssg_label = Label(my_canvas, text="Question Added Successfully!!!", bg="#5f4bd1", fg="#08ff08",
                               font=("Helvetica", 30, "bold"), anchor="nw")
...
def validate_questions_options_data():
    ...
    if (len(get_options) != 4):
        ...
        error_canvas.create_text(120, 70, text="Please Add Correct Number Of Options As Instructed",
                                 font=("Helvetica", 18, "bold"), fill="#e31313", anchor="nw")
    ...
    # To Check if Correct Option is Present in Both Rows
        ...
        error_canvas.pack(fill="both", expand=True)
        error_canvas.create_image(50, 60, image=error_img, anchor="nw")
        error_canvas.create_text(120, 70, text="Please Check If Correct Option Is Present In Both The Input Rows",
                                 font=("Helvetica", 18, "bold"), fill="#e31313", anchor="nw")
        ok_button = Button(error_canvas, image=ok_btn, cursor="hand2", borderwidth=0, command=error_validator)
